...
public class PN532 {
    ...
    public long getFirmwareVersion() throws InterruptedException {
        ...
        CommandStatus statusc = medium.writeCommand(command);
        if (statusc != CommandStatus.OK) {
            System.out.println("COMMAND STATUS NOK , got: " + statusc);
            return 0;
        }
        ...
        if (status < 0) {
            System.out.println(" READRESPOPNE STATUS NOK , got: " + status);
